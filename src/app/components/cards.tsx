...
const FlipCardComponent = () => {
    return (
        <section className="mx-auto flex justify-center gap-16  px-4 py-10 lg:max-w-7xl">
            {questions.map((question, index) => (
                <div key={index} className="group h-60 w-96">
                        ...
                        <div className="relative h-full w-full rounded-xl shadow-xl transition-all duration-500 [transform-style:preserve-3d] group-hover:[transform:rotateY(180deg)]">
                            ...
                            <div className="absolute inset-0 h-full w-full rounded-xl bg-gray-800 text-white flex items-center justify-center px-5 py-3 [transform:rotateY(180deg)] [backface-visibility:hidden]">
                                <p className="text-lg text-gray-300 mt-2">
                                    {index === 0
                                        ? "I focus on front-end development with React, but I’m also comfortable with full-stack work. My goal is to build clean, functional interfaces while understanding the full system behind them."
                                        : index === 1
